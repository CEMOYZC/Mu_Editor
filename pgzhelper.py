...
class Collide:
    ...
    def line_lines(l1x1, l1y1, l1x2, l1y2, l2):
        ...
        for l in l2:
            ...
            if determinant == 0:
                ...
                continue
            uA = (
                (l[2] - l[0]) * (l1y1 - l[1]) - (l[3] - l[1]) * (l1x1 - l[0])
            ) / determinant
    ...
    def rect_point(x, y, w, h, px, py):
        ...
        if px < x - half_w or px > x + half_w or py < y - half_h or py > y + half_h:
            return False
        return True
        ...
        for obb_line in obb_lines:
            ...
            for rect_line in rect_lines:
                ...
                if uA < 0 or uA > 1:
                    continue
                uB = (
                    l1x2_l1x1 * (l1y1 - l2y1) - l1y2_l1y1 * (l1x1 - l2x1)
                ) / determinant
                if uB < 0 or uB > 1:
                    continue
                return True
...
class Actor(Actor):
    ...
    def move_towards(
        self, target: Union[int, float, Actor, _Coordinate], dist, stop_on_target=True
    ):
    ...
    def next_image(self) -> int:
        if self._subrects is not None:
            ...
            if self.image in self._images:
                next_image_idx = (self._images.index(self.image) + 1) % len(
                    self._images
                )
                self._image_idx = next_image_idx
    ...
    def image(self, image):
        ...
        self._update_pos()
        if image is not None:
            if (image not in self._surfs) or (
                self._surfs[image][1] != self._transform_cnt
            ):
                self._transform_surf()
    ...
    def subrect(self, subrect: pygame.Rect):
        ...
        if subr != self._subrect:
            ...
            if self._subrect is not None:
                ...
                self._update_pos()
                if (surf_name not in self._surfs) or (
                    self._surfs[surf_name][1] != self._transform_cnt
                ):
                    self._transform_surf()
